...
export class CloudStorageIPCHandlers {
  ...
  register(): void {
    ...
    ipcMain.handle("rclone:install", async (event: IpcMainInvokeEvent) => {
    ...
    ipcMain.handle("rclone:test-r2-connection", async (_event, config: R2Config) => {
      return await this.testR2ConnectionUseCase.execute(config);
    });

    // R2 Server handlers
    ipcMain.handle("rclone:list-servers", async (_event, config: R2Config) => {
    ...
    ipcMain.handle("server:should-download", async (_event, config: R2Config, serverId: string) => {
      return await this.shouldDownloadServerUseCase.execute(config, serverId);
    });

    // Server Lock handlers
    ipcMain.handle("server:create-lock", async (_event, serverId: string, username: string) => {
    ...
    ipcMain.handle("server:delete-lock", async (_event, config: R2Config, serverId: string) => {
      return await this.deleteServerLockUseCase.execute(config, serverId);
    });

    ipcMain.handle("server:delete-local-lock", async (_event, serverId: string) => {
      return this.deleteLocalServerLockUseCase.execute(serverId);
    });

    // Session handlers
    ipcMain.handle("server:create-session", async (_event, serverId: string, username: string) => {
      return this.createSessionUseCase.execute(serverId, username);
    });

    ipcMain.handle("server:update-session", async (_event, serverId: string, username: string) => {
      return this.updateSessionUseCase.execute(serverId, username);
    });

    ipcMain.handle("server:upload-session", async (_event, config: R2Config, serverId: string) => {
      return await this.uploadSessionUseCase.execute(config, serverId);
    });

    ipcMain.handle("server:get-statistics", async (_event, serverId: string) => {
      return this.getServerStatisticsUseCase.execute(serverId);
    });

    // Server Properties handlers
    ipcMain.handle("server:read-port", async (_event, serverId: string) => {
      return this.readServerPortUseCase.execute(serverId);
    });

    ipcMain.handle("server:write-port", async (_event, serverId: string, port: number) => {
